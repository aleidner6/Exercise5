...
```{r, echo=FALSE}
decorate_chunk("dog1", eval=FALSE) %>% 
  flair('legend.position = "none"') %>% 
  flair("shadow_wake(wake_length = .3)") %>% 
  flair("exit_fade()") %>%
  flair("geom_text(aes(label = Breed))") %>% 
...
```{r, echo=FALSE}
knitr::include_graphics("dog1.gif")
```  
...
```{r, echo=FALSE}
decorate_chunk("covid1", eval=FALSE) %>% 
  flair('legend.position = "none"') %>% 
  flair("exit_fade()") %>%
  flair("geom_text(aes(label = state))") %>% 
  flair('subtitle = "Date: {frame_along}"') %>%
  knit_print.with_flair()
```

```{r, eval=FALSE, echo=FALSE}
anim_save("covid1.gif")
```
  
```{r, echo=FALSE}
knitr::include_graphics("covid1.gif")
```  
